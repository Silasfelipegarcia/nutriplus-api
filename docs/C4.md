...
2. Preencher perfil nutricional (onboarding).
...
| **OpenAI** | LLM usado pelo agente para montar planos (quando `USE_MOCK_LLM=false`). |
...
| `onboarding` | Salvar perfil nutricional |
...
C4Component
    ...
    Container_Boundary(api, "nutriplus-api") {
        Component(filters, "Web filters", "CorrelationIdFilter, RequestPerformanceFilter, MdcUserFilter")
...
| `TraceContext` | `infrastructure/web/TraceContext.java` | Exporta MDC para chamadas HTTP saída |
...
sequenceDiagram
    ...
    FE->>F: POST + X-Correlation-Id, X-Trace-Id, X-Flow-Id, X-Session-Id
...
| Trace estável em retry 401 (app) | Gap | Nova geração de cid/trace no retry |
